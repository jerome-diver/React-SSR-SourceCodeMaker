...
const actionsContainer = (UInormal, UIedit) => {  // actions comes after states !
    const Actions = (props) => {
        ...
        const update = e => { // via submit form button
            console.log("SUBMIT with", e)
            e.preventDefault();
            const form_to_submit = e.currentTarget;
            if (form_to_submit.checkValidity() === false) {
                console.log("wow... bijou !")
                e.stopPropagation();
            } else {
            console.log("we are there papa")
            dispatch({ type: 'update', reference: { id: response.id, body: data } })
            setValidated(true)
            setMode('normal')
            }
        }
        const remove = () => { // via "remove" ContainerLinks props callback action
            const answer = window.confirm(t('containers.delete_confirm', 
                                    { type: response.content.type_name, 
                                      title: trContainer(i18n.language, response.content).title}) )
            if (answer) {
                dispatch({ type: 'delete', reference: response.id })
                setMode('empty')
            }
        }
        ...
        switch (mode) {
            case 'edit':
                return <> <UIedit {...props} container={answer} /></>
            case 'normal':
                return <><UInormal {...props} container={answer} /></>
            default: 
                return null
        }
    }
